 ...
 *
 *  - text over a surface axe declines to resolve. On first paint this page has
 *    six of them, all in Panel 4's comparison table: the eight-column table
 *    carries a hard `min-width: 720px` inside an `overflow-x: auto`
 *    `.table-wrap`, so its right-hand cells are clipped by their own scroller
 *    and axe reports "background color could not be determined because it's
 *    partially obscured by another element" — the security-assumption and
 *    years-of-cryptanalysis columns, which are the two the whole panel exists
 *    to compare. The hero's `.cl-hero-why` aside is the other shape: a
 *    `color-mix(in oklab, var(--accent) 6%, transparent)` fill that
 *    `getComputedStyle` reports back unresolved. A violations-only gate
 ...
 *    at `.85` (the page's own subtitle) and `.btn:disabled` at `.45`, which is
 *    five of Panel 3's six buttons on arrival.
 *
 ...
 *    boxes are `.table-wrap` (`overflow-x: auto` around a `min-width: 720px`
 *    table — exactly why axe gave up on those six cells), the three
 *    `.matrix-card pre` blocks, `.mgrid` (the 8x16 generator grid), `.hex-dump`
 *    (`overflow: auto` under a 260px cap around 8 KB of hex), and `.bar-track`
 *    and `.meter` (`overflow: hidden` around their fills). Content scrolled
 *    past those is not dimmed or partly drawn — it is absent from the frame,
 *    and asking what colour it sits on has no answer. Its rect is still outside
 *    every ancestor's box, so the ancestor walk would find nothing behind it
 *    and fall through to white, inventing a failure. Skip it, and rely on the
 ...
 *    draws every figure it has in HTML and CSS — the matrix views are `<span>`
 *    cells in a flex grid, the bar chart is nested `<div>`s, the timeline is a
 *    `<ul>` with pseudo-element dots — so the sibling walk finds nothing to do.
 *    It is kept because the alternative is a helper that silently mismeasures
 *    the first labelled diagram anyone adds: an ancestor-only walk would
 *    composite a `<text>` onto the `<svg>`'s own transparent background rather
 *    than onto the `<rect>` drawn under it, and report a ratio nothing on
 *    screen has.
 *
 ...
 * rule. This page has two that carry meaning, and `nontext.ts` cannot reach
 * them either, because its generated-content check keys on a `content` STRING
 * and both of these are `content: ""` boxes coloured by `background`. They are
 * measured by hand instead: `.timeline::before`, the 2px rail down Panel 5, is
 * `--border-strong` on the panel — 3.64:1 dark, 3.50:1 light — and
 * `.timeline-item::before`, the seven dots on it, is `--primary` — 6.44:1 dark,
 * 5.27:1 light. Between them they are the entire visual structure of the
 * cryptanalysis timeline, and both clear the 3:1 SC 1.4.11 asks of a meaningful
 * graphic.
 *
 * SECOND, `aria-hidden` subtrees, which BOTH oracles skip. See the `ariaHidden`
 * helper below for the enumeration, and for the one case here that hides a real
 * generated value.
 */
 ...
 * uses. Nothing here is short-lived: `styles/main.css` declares no `@keyframes`
 * and no `animation` property, and every state change is a class or an
 * attribute that persists until the next click. The parameter is kept because
 * the narrowed form is what makes a short-lived state measurable at all if one
 * is ever added — a class that lives 500ms is shorter than a full axe pass, so
 * a whole-page scan would catch it only sometimes, and a sometimes-measured
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * that is `.table-wrap` (`overflow-x: auto` around a `min-width: 720px`
     * table), the three `.matrix-card pre` blocks, `.mgrid`, `.hex-dump`
     * (`overflow: auto` under a 260px cap) and `.bar-track` / `.meter`
     * (`overflow: hidden`). `.sr-only` is handled separately below.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // exactly the shape that guard exists for — as is this lab's own footer
      // `.github-badge svg`, whose single <path> is `fill: currentColor`. The
      // rule is the same one that would apply the moment this lab grows an SVG
      // figure with labels of its own.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // at `top: -3rem` and the lab's own `.skip-link` at `top: -100%`, each
      // sliding in only on focus. Measuring the parked copies invents failures
      // for text that is not on screen; both focused renderings are real states
      // and the gate scans each of them explicitly instead.
      ...
      //
      // This lab uses the idiom for real, TWICE, which is why the guard is
      // here and not merely inherited. `styles/main.css` defines both
      // `.sr-only` and `.aria-status` as `position:absolute; width:1px;
      // height:1px; clip:rect(0,0,0,0)`. `.sr-only` carries the two table
      // `<caption>`s and the `<label>` for the hex dump; `.aria-status` is
      // `#aria-live-status`, an `aria-live="polite"` region whose text changes
      // on EVERY action this drive takes. Each has a rect and passes
      // `checkVisibility()` while painting exactly zero pixels, so measuring
      // them reports ratios for ink that was never laid down — and
      // `#aria-live-status` would report a fresh one at every single scan. It
      // is deliberately narrow: only a ZERO-AREA clip qualifies, so a partially
     ...
     *
     * What this lab hides is a closed set: the `.step-number` badges (whose
     * number is repeated in the adjacent `.step-title`), every `.bit-cell`
     * digit inside a non-editable `.bit-vector` and every `.mgrid-cell` — both
     * of which sit inside a `role="img"` whose `aria-label` spells the whole
     * vector or matrix out — the shared header's hamburger and GitHub marks and
     * the footer's GitHub mark, and the three `<pre>` matrix dumps.
     *
     * The `<pre>` dumps are the ONE place this page hides a real generated
     * VALUE, which is the shared blind spot both oracles stop at, so they were
     * measured by hand rather than waved through: `--text-secondary` on `--bg`,
     * 9.17:1 dark and 8.47:1 light. Hiding them is a deliberate teaching call —
     * `renderBinMatrix`'s own comment reads "Large binary matrices are
     * decorative; the card heading describes them" — and the same matrices are
     * exposed as a summarised `role="img"` grid immediately below. Nothing else
     * hidden here carries a value the exposed text does not also state.
     */
     ...
     *
     * The only SVG on this page is glyph-shaped and text-free: the shared
     * header's hamburger and GitHub marks and the footer's GitHub mark. The
     * guard stays because the usual way the mistake arrives is exactly
     ...
     * declares no background at all, so the canvas value is taken from `body`,
     * which sets `background: var(--bg)` and `min-height: 100vh` — a box that
     * grows with the document rather than being pinned to one viewport. The
     * guard stays because this page is five panels and many screens deep, so
     * the day a `height: 100%` replaces that `min-height`, everything below the
     * first screen would report against a white page that does not exist, in a
     * theme whose `--bg` is `#0d0d1a`.
     */
